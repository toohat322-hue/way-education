  ...
  return (
    <AdminModal title={major ? "Edit Major" : "Add Major"} onClose={guardedClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Name (English)"><TextInput required value={form.nameEn} onChange={set("nameEn")} /></Field>
        <Field label="Name (Arabic)"><TextInput required value={form.nameAr} onChange={set("nameAr")} dir="rtl" /></Field>
        <Field label="Program count"><TextInput type="number" value={form.count} onChange={set("count")} /></Field>
        <div>
          <Label>Icon</Label>
...
export default function AdminMajors() {
  const { majors, removeMajor, resetMajors } = useData();
  const showToast = useToast();
