...
import { GraduationCap, BookOpen, Users, Trophy, Star, Sparkles } from "lucide-react";
 ...
 */
export default function StudentAnimation({ className = "", speed = 1, loop = true, autoCelebrate = true }) {
  ...
  const badges = [
    { icon: BookOpen, label: "Studying", sub: "Focused sessions", grad: grad.card1, corner: "top-2 -left-2 md:left-2" },
    { icon: Users, label: "Collaborating", sub: "Study groups", grad: grad.card2, corner: "top-6 -right-2 md:right-2" },
    { icon: Trophy, label: "Celebrating", sub: "Goals reached", grad: grad.card3, corner: "bottom-4 -left-2 md:left-6" },
    { icon: Star, label: "Rated 4.9", sub: "By students", grad: grad.card4, corner: "bottom-0 -right-2 md:right-6" },
  ];
  ...
  return (
    ...
    >
      <div className="relative h-[320px] sm:h-[380px] md:h-[440px] max-w-md mx-auto">
        ...
        <div className="absolute inset-6 rounded-[32px]" style={{ background: grad.primary, opacity: 0.1 }} />
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative flex items-center justify-center">
            ...
            <div
              ...
              style={{ background: grad.primary, boxShadow: "0 20px 45px rgba(41,82,227,0.35)" }}
            >
        ...
        {badges.map(({ icon: Icon, label, sub, grad: badgeGrad, corner }, i) => (
          <GlassCard
            key={label}
            className={`absolute ${corner} p-3 w-36 sm:w-40 float-card ${inView ? "fade-up" : "opacity-0"}`}
            style={{ "--r": `${i % 2 === 0 ? -3 : 3}deg`, animationDelay: `${i * 0.6}s` }}
          >
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: badgeGrad }}>
                <Icon size={16} color="#fff" />
              </div>
              <div className="min-w-0">
                <div className="text-xs font-semibold truncate" style={{ color: C.ink }}>{label}</div>
      ...
      <div className="flex justify-center mt-4">
        <button
          ...
          style={{ color: "#fff", background: grad.cta, boxShadow: "0 8px 20px rgba(255,106,43,0.3)" }}
        >
