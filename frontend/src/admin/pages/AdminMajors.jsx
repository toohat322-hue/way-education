...
import { PageHeader, Field, Label, TextInput, PrimaryButton, GhostButton, DangerButton, AdminModal } from "../ui";

function initFromMajor(major) {
  if (major) {
    return { nameEn: major.name.en, nameAr: major.name.ar, count: major.count, iconName: major.iconName || "GraduationCap" };
  }
...
function MajorFormModal({ major, onClose }) {
  ...
  return (
    <AdminModal title={major ? "Edit Major" : "Add Major"} onClose={guardedClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Name (English)" id="major-name-en"><TextInput required value={form.nameEn} onChange={set("nameEn")} /></Field>
        <Field label="Name (Arabic)" id="major-name-ar"><TextInput required value={form.nameAr} onChange={set("nameAr")} dir="rtl" /></Field>
        <Field label="Program count" id="major-program-count"><TextInput type="number" value={form.count} onChange={set("count")} /></Field>
        <div>
          <Label>Icon</Label>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <PrimaryButton type="submit">{major ? "Save changes" : "Add major"}</PrimaryButton>
        </div>
      </form>
    </AdminModal>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {majors.map((m) => {
          ...
          return (
            <GlassCard key={m.id} className="p-5" style={{ background: "#fff" }}>
              ...
              <div className="font-semibold text-sm mb-1" style={{ color: C.ink }}>{m.name.en}</div>
              <div className="text-xs mb-3" style={{ color: C.muted }}>{m.count} programs</div>
              <div className="flex items-center gap-2">
                <GhostButton onClick={() => setEditing(m)} className="flex-1 flex items-center justify-center gap-1.5">
                  <Pencil size={13} /> Edit
                </GhostButton>
                <DangerButton aria-label={`Delete ${m.name.en}`} onClick={async () => { if (window.confirm(`Remove ${m.name.en}?`)) { try { await removeMajor(m.id); showToast(`${m.name.en} removed`); } catch (err) { showToast(err.message || `Unable to remove ${m.name.en}`, "error"); } } }}>
                  <Trash2 size={13} />
                </DangerButton>
              </div>
            </GlassCard>
      ...
      {editing !== undefined && <MajorFormModal major={editing} onClose={() => setEditing(undefined)} />}
    </div>
  );
}
