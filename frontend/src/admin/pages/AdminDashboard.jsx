...
const SECTIONS = [
  {
    ...
    icon: Building2,
    desc: "Full detail pages: tuition, majors, scholarships, documents, reviews.",
  },
  {
    to: "/admin/directory",
    label: "Directory",
    icon: ListTree,
    desc: "The full public directory of universities shown on /universities.",
  },
  {
    to: "/admin/majors",
    label: "Majors",
    icon: BookOpen,
    desc: "The popular-majors grid on the homepage.",
  },
  {
    to: "/admin/faqs",
    label: "FAQs",
    icon: HelpCircle,
    desc: "Frequently asked questions section.",
  },
  {
    to: "/admin/content",
    label: "Site Copy",
    icon: Type,
    desc: "Every EN/AR string used across the site.",
  },
  {
    to: "/admin/blog",
    label: "Blog",
    icon: FileText,
    desc: "Manage articles, news, and guides.",
  },
  {
    to: "/admin/seo",
    label: "SEO",
    icon: Search,
    desc: "Manage page metadata, OpenGraph tags, and schema.",
  },
  {
    to: "/admin/leads",
    label: "Leads",
    icon: Inbox,
    desc: "CRM pipeline for incoming consultations and applications.",
  },
];
...
export default function AdminDashboard() {
  ...
  const handleImportFile = (e) => {
    ...
    if (
      !window.confirm(
        "Importing will replace all live backend data with the backup file. Continue?",
      )
    )
      return;
  ...
  return (
    <div>
      <PageHeader
        title="Dashboard"
        sub="Manage everything shown on Way Education — changes persist through the backend API."
      />
      ...
      <GlassCard className="p-5 mb-8" style={{ background: "#fff" }}>
        ...
        <div className="text-xs mb-4 max-w-md" style={{ color: C.muted }}>
          Controls the floating WhatsApp button and every "Chat on WhatsApp"
          link across the site.
        </div>
        <div className="flex flex-wrap items-end gap-3 max-w-md">
          <div className="flex-1 min-w-[180px]">
            <Label htmlFor="dashboard-whatsapp">
              Number (country code + number, digits only)
            </Label>
            <TextInput
      ...
      <GlassCard
        className="p-5 mb-8 flex flex-wrap items-center justify-between gap-4"
      ...
      >
        <div>
          ...
          <div className="text-xs max-w-md" style={{ color: C.muted }}>
            Export the current CMS snapshot for safekeeping or migration. Import
            will replace the live backend content with the snapshot.
          </div>
        </div>
