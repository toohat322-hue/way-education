    ...
    <div>
      ...
      <GlassCard className="p-5 mb-8" style={{ background: "#fff" }}>
        ...
        <div className="flex flex-wrap items-end gap-3 max-w-md">
          <div className="flex-1 min-w-[180px]">
            <Label htmlFor="dashboard-whatsapp">Number (country code + number, digits only)</Label>
            <TextInput id="dashboard-whatsapp" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="905000000000" />
          </div>
      ...
      <GlassCard className="p-5 mb-8 flex flex-wrap items-center justify-between gap-4" style={{ background: "#fff" }}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <GhostButton onClick={handleExport} className="flex items-center gap-1.5" aria-label="Export CMS backup">
            <Download size={14} /> Export
          </GhostButton>
          <PrimaryButton onClick={handleImportClick} className="flex items-center gap-1.5" aria-label="Import CMS backup">
            <Upload size={14} /> Import
          </PrimaryButton>
          <input ref={fileInputRef} type="file" accept="application/json" className="hidden" onChange={handleImportFile} aria-label="Select backup file to import" />
        </div>
      </GlassCard>
