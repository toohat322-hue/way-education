...
// Order matches t.nav in translations.js: [Universities, Programs, About, Contact]
const NAV_ROUTES = ["/universities", "/#programs", "/about", "/contact"];
    ...
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 md:h-20 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 shrink-0">
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
            <img src="/brand/logo.jpeg" alt="Way Education logo" className="w-full h-full object-contain" />
          </div>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {t.nav.map((item, i) => {
            const isActive = location.pathname === NAV_ROUTES[i] || (location.pathname + location.search === NAV_ROUTES[i]) || (location.pathname + location.hash === NAV_ROUTES[i]);
            return (
            <Link
              key={i}
              to={NAV_ROUTES[i]}
              className="text-sm font-medium transition-colors"
              style={{ color: isActive ? C.blue : C.inkSoft, fontWeight: isActive ? "600" : "500" }}
              onMouseOver={(e) => (e.currentTarget.style.color = C.blue)}
              onMouseOut={(e) => (e.currentTarget.style.color = isActive ? C.blue : C.inkSoft)}
            >
              {item}
            </Link>
            );
      ...
      {menuOpen && (
        <div className="lg:hidden px-5 pb-5 flex flex-col gap-4 fade-up" style={{ borderTop: `1px solid ${C.border}` }}>
          {t.nav.map((item, i) => {
            const isActive = location.pathname === NAV_ROUTES[i] || (location.pathname + location.search === NAV_ROUTES[i]) || (location.pathname + location.hash === NAV_ROUTES[i]);
            return (
            <Link key={i} to={NAV_ROUTES[i]} onClick={() => setMenuOpen(false)} className="text-sm pt-3" style={{ color: isActive ? C.blue : C.inkSoft, fontWeight: isActive ? "600" : "500" }}>
              {item}
            </Link>
            );
          })}
