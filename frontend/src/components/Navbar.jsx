    ...
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 md:h-20 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 shrink-0">
          <div
            className="w-12 h-12 md:w-16 md:h-16 rounded-2xl overflow-hidden"
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {t.nav.map((item, i) => {
            const isActive =
              location.pathname === NAV_ROUTES[i] ||
              location.pathname + location.search === NAV_ROUTES[i] ||
              location.pathname + location.hash === NAV_ROUTES[i];
            return (
              <Link
                key={i}
                to={NAV_ROUTES[i]}
                ...
                onMouseOut={(e) =>
                  (e.currentTarget.style.color = isActive ? C.blue : C.inkSoft)
                }
              >
                {item}
              </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            ...
            style={{
              background: grad.cta,
              boxShadow: "0 8px 20px rgba(255,106,43,0.35)",
            }}
          >
            {t.navApply}
          </button>
      ...
      {menuOpen && (
        <div
          className="lg:hidden px-5 pb-5 flex flex-col gap-4 fade-up"
        ...
        >
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <button
              onClick={() => navigate("/contact?source=mobile-navbar-apply")}
              className="flex-1 px-5 py-2.5 rounded-full text-sm font-semibold text-white"
